...
export class SingleRecipe extends React.Component {
  handleNewRecipeSubmit(recipeId) {
    this.props
      .dispatch(
        postRecipeToDatabase(recipeId, this.props.userId, this.props.authToken)
      )
      .then(() =>
        this.props.dispatch(
          updateStateWithDatabaseResults(
  ...
  render() {
    ...
    const arrayOfRecipesFromSavedRecipes = this.props.recipes.map(
    ...
    if (this.props.viewingSingleItem) {
      ...
      if (currentItem.analyzedInstructions[0]) {
        instructions = currentItem.analyzedInstructions[0].steps.map(
          (item, index) => {
            return (
              <div key={index}>
                {" " + (index + 1) + ". " + item.step + "\n"}
      ...
      return (
        <div className="recipeOverview">
          ...
          <img
            className="sigleRecipeImage"
            src={currentItem.image}
            alt={currentItem.title}
          />
          <div className="recipeInstructions">{instructions}</div>

          <a href={currentItem.sourceUrl} target="blank" className="recipeLink">
            Full Recipe
          </a>
          {this.props.loggedIn &&
          !arrayOfRecipesFromSavedRecipes.includes(currentItem.id) ? (
            <button
              onClick={() => this.handleNewRecipeSubmit(currentItem.id)}
              className="singleRecipeUserSaveButton"
            >
              Save
            </button>
          ) : this.props.loggedIn &&
            arrayOfRecipesFromSavedRecipes.includes(currentItem.id) ? (
            <button
              onClick={() => this.handleRemoveRecipeSubmit(currentItem.id)}
              className="singleRecipeUserSaveButton"
            >
              UnSave
            </button>
